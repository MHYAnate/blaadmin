      ...
      <div className="w-full max-w-4xl bg-white rounded-lg p-8 shadow-sm max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex flex-col md:flex-row items-start gap-6 p-6 bg-gray-50 rounded-lg">
          ...
          <div className="flex-1">
            ...
            <p className="text-gray-600 mb-3">{order.totalQuantity} item(s)</p>
...
const getDisplayStatus = (status: string) => {
  const ongoingStatuses = ['PENDING', 'SCHEDULED', 'PROCESSING', 'SHIPPED', 'COMPLETED'];
...
export default function DetailedOrderTable() {
  ...
  const [orderStatusFilter, setOrderStatusFilter] = useState<string>("DELIVERED");
  ...
  useEffect(() => {
    ...
    if (customerTypeFilter) payload.customerType = customerTypeFilter;
    
    if (orderStatusFilter) {
      if (orderStatusFilter === "ONGOING") {
        payload.status = "PENDING,SCHEDULED,PROCESSING,SHIPPED,COMPLETED";
      } else {
        payload.status = orderStatusFilter;
      }
    }
    
    if (startDate) payload.dateFrom = startDate;
    if (endDate) payload.dateTo = endDate;
    ...
    <div className="w-full max-w-7xl mx-auto p-8 bg-gray-50">
      ...
      <div className="bg-white rounded-lg shadow-sm">
        <div className="p-6 border-b border-gray-200">
          ...
          Detailed Delivered Order Table
          </h1>
        </div>
        ...
        <div className="flex items-center gap-4 text-sm font-medium text-slate-700">
          <button
            className="border border-slate-300 bg-white hover:bg-slate-100 text-slate-700 rounded-md px-4 py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={() =>
          ...
          <button
            className="border border-slate-300 bg-white hover:bg-slate-100 text-slate-700 rounded-md px-4 py-2 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={() =>
              onPageChange(currentPage !== null ? currentPage + 1 : 1)
            }
          >
            Next →
          </button>
